...
These properties control how Tycho resolves the dependencies of a project against its target platform.

Name | Value | Default | Documentation
--- | --- | --- | ---
tycho.p2.resolver.max-threads | number | number of available processors | Maximum number of target environments resolved in parallel. When a project is configured for several target environments (e.g. a product or feature built for multiple os/ws/arch), each environment is resolved by an independent SAT solve; these solves are CPU-bound and run concurrently over a bounded pool. Set to `1` to resolve environments one after another (the previous behavior). This pool is separate from the network-tuned download pool (`tycho.p2.transport.max-download-threads`). Single-environment projects are unaffected.
